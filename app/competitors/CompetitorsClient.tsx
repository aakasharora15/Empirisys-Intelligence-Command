...
const CompetitorCharts = dynamic(
  () => import('@/components/competitors/CompetitorCharts').then((mod) => mod.CompetitorCharts),
  { ssr: false },
);
import { Competitor, CompetitorContent, DiscoveryLog } from '@/lib/db';
    ...
    try {
      ...
      fetch('/api/query', {
        ...
        body: JSON.stringify({
          ...
          results_json: { response: fullText },
        }),
      ...
      <div id="competitor-module-container" className="w-full px-6 md:px-10 mt-8 space-y-8">
        ...
        <div className="-mt-16 md:-mt-20 relative z-20 mb-8 glass-card px-6 py-5 rounded-2xl shadow-sm grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-6 divide-y md:divide-y-0 md:divide-x divide-card-border bg-card">
          {[
            ...
            <div
              key={i}
              className="flex flex-col justify-center px-4 py-2 first:pt-0 md:first:pt-2"
            >
              <span className="text-micro font-bold text-text-secondary uppercase tracking-wider mb-2">
                {metric.label}
              </span>
              <span className="text-3xl font-black text-text-primary">{metric.val}</span>
            </div>
          ))}
        </div>
        ...
        <div className="glass-card p-6 rounded-2xl shadow-sm space-y-4">
          ...
          <form
            ...
            className="flex items-center gap-3 bg-background border border-card-border p-1.5 rounded-full"
          >
            <input
              type="text"
              value={aiInput}
          ...
          {streamedText && (
            <div className="space-y-4 animate-fadeIn">
              <div className="p-4 bg-background rounded-2xl border border-card-border text-xs text-text-primary leading-relaxed relative">
                <span className="absolute top-2 right-2 text-accent font-serif italic font-bold">
                  RAI Response
                </span>
                {streamedText}
                {isStreaming && (
                  <span className="inline-block h-3.5 w-1 bg-accent animate-pulse ml-0.5" />
                )}
              </div>
              ...
              {!isStreaming && (
                <div className="p-4 bg-accent/10 border-l-4 border-l-accent rounded-r-2xl">
                  ...
                  <p className="text-xs font-semibold text-text-primary">
                    Focus sales arguments on BOOST real time NLP ingestion to render competitor
                    legacy forms obsolete.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="flex justify-between items-center glass-card p-4 rounded-2xl shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <PdfExportButton
              targetId="competitor-module-container"
              filename="competitor-intelligence-report"
            />
          </div>
        </div>
        ...
        {viewMode === 'cards' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredComps
              .filter((c) => c.status === 'TRACKED')
              .map((comp) => {
                const borderCol =
                  comp.threat_score >= 80
                    ? 'border-t-[#EF4444]'
                      ...
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-accent/10 flex items-center justify-center text-accent font-bold shadow-sm overflow-hidden border border-card-border relative bg-white">
                          <Image
                            ...
                            src={
                              comp.logoUrl ||
                              `https://logo.clearbit.com/${comp.website.replace('https://', '').replace('http://', '').replace('www.', '').split('/')[0]}`
                            }
                            alt={comp.name}
            ...
            {discoveryLogs.length === 0 ? (
              <div className="col-span-1 md:col-span-2 glass-card p-12 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-text-secondary max-w-md">
                  Your automated AI discovery pipeline is active and monitoring the market. Any
                  newly detected competitors will appear here for your review.
                </p>
              </div>
            ) : (
              discoveryLogs.map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="glass-card rounded-2xl p-6 shadow-sm border-l-4 border-l-[var(--accent)] relative overflow-hidden group"
                  >
                    {log.status === 'EMERGING' ? (
                      <span className="absolute top-0 right-0 bg-[var(--accent)] text-white text-micro font-bold px-3 py-1 uppercase tracking-wider rounded-bl-lg">
                        Emerging
                      </span>
                    ) : (
                      <span className="absolute top-0 right-0 bg-accent text-white text-micro font-bold px-3 py-1 uppercase tracking-wider rounded-bl-lg">
                        Watchlist
                      </span>
                    )}
                    ...
                    <div className="mb-4">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-micro text-text-secondary hover:text-accent flex items-center gap-1"
                      >
                    ...
                    <div className="space-y-4">
                      <div>
                        <span className="text-micro font-bold text-text-secondary uppercase tracking-wider block mb-1">
                      ...
                      <div className="bg-background border border-card-border p-3 rounded-lg flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-micro text-text-secondary mt-1">
                            First detected: {log.detected_at} via {log.source}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-card-border">
                        <div className="flex flex-col">
                          <span className="text-micro font-bold text-text-secondary uppercase tracking-wider mb-1">
                            Est Threat Score
                          </span>
                          <span
                            className={cn(
                              'px-2 py-0.5 rounded text-xs font-bold text-center inline-block w-max',
                              compData.threat_score >= 80
                                ? 'bg-[#EF4444]/10 text-[#EF4444]'
                                : compData.threat_score >= 65
                                  ? 'bg-accent/10 text-accent'
                                  : 'bg-accent/10 text-accent',
                            )}
                          >
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              ...
                              const updatedComps = competitors.map((c) =>
                                c.id === compData.id ? { ...c, status: 'DISMISSED' as const } : c,
                              );
                          ...
                          <button
                            onClick={() => {
                              ...
                              setDiscoveryLogs(updatedLogs);
                              const updatedComps = competitors.map((c) =>
                                c.id === compData.id ? { ...c, status: 'TRACKED' as const } : c,
                              );
                              setCompetitors(updatedComps);
                              setViewMode('matrix');
                            }}
