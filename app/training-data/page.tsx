...
import {
  ...
  SFIcloudAndArrowUp as UploadCloud,
  SFTextDocument as FileText,
  SFTablecells as FileSpreadsheet,
...
const INITIAL_LIBRARY = [
  {
    id: 'doc_1',
    name: 'Empirisys_Q3_Strategic_Playbook.pdf',
    type: 'PDF',
    date: '2026-06-15',
    size: '4.2 MB',
    status: 'ACTIVE',
  ...
  {
    id: 'doc_2',
    name: 'Competitor_Battlecards_2026.pptx',
    type: 'PPTX',
    date: '2026-06-12',
    size: '12.8 MB',
    status: 'ACTIVE',
  ...
  {
    id: 'doc_3',
    name: 'HSE_Software_Market_Analysis.docx',
    type: 'DOCX',
    date: '2026-06-10',
    size: '2.1 MB',
    status: 'ACTIVE',
  ...
  {
    id: 'doc_4',
    name: 'DNV_vs_Sphera_Pricing_Matrix.xlsx',
    type: 'XLSX',
    date: '2026-06-05',
    size: '840 KB',
    status: 'ACTIVE',
        ...
        belowContent={
          <div className="flex flex-col md:flex-row gap-6 mt-8">
            ...
            <div className="glass-card flex-1 p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-micro font-bold text-text-secondary uppercase tracking-wider">
                  Active Documents
                </p>
                <p className="text-xl font-bold text-text-primary">
                  {library.filter((d) => d.status === 'ACTIVE').length}
                </p>
              </div>
            </div>
            <div className="glass-card flex-1 p-4 flex items-center gap-4">
      ...
      <div className="w-full px-6 md:px-10 space-y-8 max-w-[1600px] mx-auto relative z-20">
        ...
        >
          ...
          <p className="text-xs text-text-secondary max-w-md relative z-10">
            Upload PDF, Word, PowerPoint, or Excel files. The AI will automatically chunk,
            vectorize, and embed these documents into its knowledge graph to improve future insights
            and outputs.
          </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead className="bg-[#060D0E] border-b border-card-border">
                <tr>
                  <th className="p-4 text-micro font-bold text-text-secondary uppercase tracking-wider w-[40%]">
                    Document Name
                  </th>
                  <th className="p-4 text-micro font-bold text-text-secondary uppercase tracking-wider">
                    Type
                  </th>
                  <th className="p-4 text-micro font-bold text-text-secondary uppercase tracking-wider">
                    Upload Date
                  ...
                  <th className="p-4 text-micro font-bold text-text-secondary uppercase tracking-wider text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {filteredLibrary.length === 0 ? (
                  ...
                  filteredLibrary.map((doc) => (
                    <tr key={doc.id} className="hover:bg-card/30 transition-colors group">
                      ...
                      <td className="p-4 text-xs text-text-secondary">{doc.date}</td>
                      <td className="p-4 text-xs text-text-secondary">{doc.size}</td>
                      <td className="p-4">
                        {doc.status === 'ACTIVE' ? (
                          <div className="flex items-center gap-1.5">
                            <CheckCircle2 className="h-4 w-4 text-accent" />
                            <span className="text-micro font-bold text-accent uppercase tracking-wider">
                              Active in Context
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-micro font-bold text-[#DC2626] uppercase tracking-wider">
                              Failed
                            </span>
                          </div>
                        )}
