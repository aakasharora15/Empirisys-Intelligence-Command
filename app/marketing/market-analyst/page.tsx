...
const tierColors: Record<string, { bg: string; text: string; label: string }> = {
  A: {
    ...
    label: 'Tier A · Regulatory',
  },
  B: {
    bg: 'bg-blue-500/10 border-blue-500/20',
    text: 'text-blue-600 dark:text-blue-400',
    label: 'Tier B · Procurement',
  },
  C: {
    bg: 'bg-amber-500/10 border-amber-500/20',
    text: 'text-amber-600 dark:text-amber-400',
    label: 'Tier C · Trade Press',
  },
  D: {
    bg: 'bg-panel0/10 border-zinc-500/20',
    text: 'text-text-secondary',
    label: 'Tier D · Business Press',
  },
};
...
const deltaLabels: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  faded: {
    color: 'text-text-secondary bg-card-border/50',
...
export default function MarketAnalystPage() {
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'themes' | 'events' | 'landscape'>(
    'dashboard',
  );
  ...
  const handleRunPipeline = async () => {
    ...
    try {
      ...
      if (
        e.message?.includes('AI_PROVIDER_NOT_CONFIGURED') ||
        (e.response && e.response.status === 500)
      ) {
        alert(
          'No AI provider key is configured, so the real-time pipeline cannot run. Set OPENAI_API_KEY or ANTHROPIC_API_KEY in your Vercel environment variables.',
        );
      }
      console.error(e);
      toast({
  ...
  const pendingThemes =
    result?.themes.filter((t) => (validationState[t.id] || t.status) === 'pending_review') ?? [];
              ...
              {[
                {
                  ...
                  value: result.meta.totalEventsExtracted,
                ...
                },
                {
                  ...
                  value: result.meta.totalSegments,
                ...
                {
                  label: 'Pipeline Run',
                  value: new Date(result.meta.generatedAt).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                  }),
                  bg: 'bg-panel0/10',
                ...
                >
                  <div
                    className={`absolute top-0 right-0 w-24 h-24 ${kpi.bg} rounded-full blur-2xl -translate-y-1/2 translate-x-1/3 group-hover:scale-150 transition-transform duration-700`}
                  ></div>
                  <div className="relative z-10">
                    <span className="text-[10px] font-bold tracking-widest text-text-secondary  uppercase">
                      {kpi.label}
                    </span>
                    <div className={`${kpi.textClass || 'text-3xl mt-1'} font-bold ${kpi.color}`}>
                      {kpi.value}
                    </div>
                  </div>
                </motion.div>
            ...
            {activeView === 'dashboard' && result.metrics && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <motion.div
                    variants={itemVariants}
                    className="bg-card backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-card-border shadow-xl shadow-sm relative overflow-hidden"
                  >
                    ...
                    <div className="h-[300px] w-full relative z-10">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            {result.metrics.budgetAllocation.map((entry: any, index: number) => {
                              ...
                              return (
                                <Cell
                                  ...
                                  style={{
                                    filter: `drop-shadow(0px 0px 8px ${colors[index % colors.length]}40)`,
                                  }}
                                />
                              );
                            })}
                          ...
                          <Tooltip
                            contentStyle={{
                              backgroundColor: 'rgba(26,21,37,0.8)',
                  ...
                  >
                    ...
                    <h3 className="text-lg font-bold text-text-primary mb-6 uppercase tracking-wider flex items-center gap-2 relative z-10">
                      <Shield className="h-5 w-5 text-[var(--color-primary)]" /> Competitive
                      Positioning Matrix
                    </h3>
                    <div className="h-[300px] w-full relative z-10">
                      <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                          ...
                          <Tooltip
                            cursor={{ strokeDasharray: '3 3', opacity: 0.1 }}
                            contentStyle={{
                              backgroundColor: 'rgba(26,21,37,0.8)',
                              backdropFilter: 'blur(12px)',
                              borderColor: 'rgba(255,255,255,0.1)',
                          ...
                          <Scatter
                            name="Competitors"
                            data={result.metrics.competitorPositioning}
                            fill="#8b5cf6"
                          >
                            {result.metrics.competitorPositioning.map(
                              (entry: any, index: number) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
                                    entry.competitorName === 'Empirisys' ? '#06b6d4' : '#8b5cf6'
                                  }
                                  style={{
                                    filter: `drop-shadow(0px 0px 10px ${entry.competitorName === 'Empirisys' ? '#06b6d4' : '#8b5cf6'})`,
                                  }}
                                />
                              ),
                            )}
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-text-primary mb-8 uppercase tracking-wider flex items-center gap-2 relative z-10">
                    <Activity className="h-5 w-5 text-[var(--color-primary)]" /> STEEPLE Trend
                    Velocity (7 Days)
                  </h3>
                  <div className="h-[400px] w-full relative z-10">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={result.metrics.trendVelocity.map((d: any) => {
                          let dataObj = d;
                ...
                {result.events.map((event, idx) => (
                  ...
                  >
                    ...
                    <div className="shrink-0">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-lg border text-[10px] font-bold uppercase ${tierColors[event.sourceTier]?.bg} ${tierColors[event.sourceTier]?.text}`}
                      >
                        {event.sourceTier}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-[11px]">
                        ...
                        <span className="text-text-secondary">🏢 {event.entityName}</span>
                        <span className="text-text-secondary">📰 {event.sourceName}</span>
                        ...
                        <div className="flex gap-2 ml-auto">
                          <span className="px-2 py-0.5 rounded bg-[var(--color-primary)]/10 text-[var(--color-primary)] font-bold">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-sm font-bold text-text-primary mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Target className="h-4 w-4 text-emerald-500" /> Discovered Market Segments (
                    {result.landscape.segments.length})
                  </h3>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <span
                              className={`px-2.5 py-1 rounded-lg border text-[10px] font-bold ${config.bg} ${config.text}`}
                            >
                              {config.label}
                            </span>
                          </div>
                ...
                <div>
                  <h3 className="text-sm font-bold text-text-primary mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Clock className="h-4 w-4 text-[var(--color-primary)]" /> Growth Strategy
                    Horizons
                  </h3>
                    ...
                    {result.landscape.horizons.map((horizon) => {
                      const horizonConfig = {
                        now: {
                          ...
                          badge: 'bg-emerald-500 text-text-primary',
                          icon: '🚀',
                        },
                        next: {
                          accent: 'from-amber-500 to-orange-500',
                          glow: 'shadow-amber-500/10',
                          badge: 'bg-amber-500 text-text-primary',
                          icon: '⚙️',
                        },
                        later: {
                          accent: 'from-zinc-400 to-zinc-500',
                          glow: 'shadow-sm',
                          badge: 'bg-panel0 text-text-primary',
                          icon: '⏸️',
                        },
                      };
                      ...
                      return (
                        ...
                        >
                          {/* Horizon Header */}
                          <div
                            className={`bg-gradient-to-r ${config.accent} p-4 text-text-primary`}
                          >
                          ...
                          <div className="p-4 space-y-3">
                            {horizon.actions?.map((action, aIdx) => (
                              <div
                                key={aIdx}
                                className="bg-card/20 rounded-xl p-3 border border-card-border"
                              >
                            ...
                            {(!horizon.actions || horizon.actions.length === 0) && (
                              <p className="text-[12px] text-text-secondary italic text-center py-4">
                                No actions identified for this horizon.
                              </p>
                            )}
                          </div>
                        </div>
            ...
            {activeView === 'landscape' && !result.landscape && (
              <div className="bg-card/40 backdrop-blur-md rounded-2xl p-12 border border-card-border text-center">
                ...
                <h3 className="text-lg font-bold text-text-primary mb-2">
                  Landscape Analysis Unavailable
                </h3>
                <p className="text-sm text-text-secondary max-w-md mx-auto">
                  The market landscape analysis could not be generated in this pipeline run. Try
        ...
        {!result && !isLoading && (
          <div className="bg-card/40 backdrop-blur-md rounded-3xl p-16 border border-card-border text-center">
            ...
            <p className="text-sm text-text-secondary max-w-md mx-auto mb-8">
              Run the pipeline to extract structured events from tiered sources, apply deterministic
              scoring, aggregate into strategic themes, and generate actionable intelligence.
            </p>
            <div className="flex flex-wrap justify-center gap-3 text-[11px] text-text-secondary">
              {[
                ...
                'Tier B: Procurement',
                'Tier C: Trade Press',
                'Tier D: Business Press',
              ].map((t) => (
                <span
                  key={t}
                  className="px-3 py-1.5 rounded-full border border-card-border bg-panel"
                >
                  {t}
                </span>
              ))}
...
function ThemeCard({
  ...
  return (
    <div
      className={`bg-card/40 backdrop-blur-md rounded-2xl border transition-all duration-300 overflow-hidden ${
        isPending
    ...
    >
      ...
      <button onClick={onToggle} className="w-full text-left p-5 flex items-start gap-4">
        {/* Gradient bar */}
        <div className="w-1.5 h-12 bg-[var(--color-primary)] rounded-full shrink-0 mt-0.5" />

        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-1.5">
            <h4 className="text-[15px] font-bold text-text-primary leading-tight">{theme.title}</h4>
            <div className="flex items-center gap-2 shrink-0">
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase ${delta.color}`}
              >
          ...
          <p className="text-[13px] text-text-secondary line-clamp-2">{theme.description}</p>
          <div className="flex items-center gap-4 mt-2 text-[11px] text-text-secondary">
            <span>
              📊 Relevance:{' '}
              <strong className="text-[var(--color-primary)]">{theme.relevanceScore}</strong>
            </span>
          ...
          <div className="bg-card/20 rounded-xl p-4 border border-card-border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-red-500/5 p-2 rounded border border-red-500/10">
                    ...
                    <p className="text-[11px] text-text-secondary">
                      {theme.interpretation?.scenarioForecasts?.bearCase}
                    </p>
                  </div>
                  <div className="bg-blue-500/5 p-2 rounded border border-blue-500/10">
                    ...
                    <p className="text-[11px] text-text-secondary">
                      {theme.interpretation?.scenarioForecasts?.baseCase}
                    </p>
                  </div>
                  <div className="bg-emerald-500/5 p-2 rounded border border-emerald-500/10">
                    <span className="text-[9px] font-bold text-emerald-500 block mb-1">
                      BULL CASE
                    </span>
                    <p className="text-[11px] text-text-secondary">
                      {theme.interpretation?.scenarioForecasts?.bullCase}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-3 rounded-lg bg-card/40 backdrop-blur-md border border-card-border">
              <span className="text-[10px] font-bold text-[var(--color-primary)] uppercase tracking-wider block mb-2">
                ...
                {theme.interpretation?.vrioAnalysis?.competitiveImplication})
              </span>
              <div className="grid grid-cols-2 gap-2 text-[11px] text-text-secondary">
                <div>
                  <strong className="text-text-primary ">V:</strong>{' '}
                  {theme.interpretation?.vrioAnalysis?.valuable}
                </div>
                <div>
                  <strong className="text-text-primary ">R:</strong>{' '}
                  {theme.interpretation?.vrioAnalysis?.rare}
                </div>
                <div>
                  <strong className="text-text-primary ">I:</strong>{' '}
                  {theme.interpretation?.vrioAnalysis?.inimitable}
                </div>
                <div>
                  <strong className="text-text-primary ">O:</strong>{' '}
                  {theme.interpretation?.vrioAnalysis?.organization}
                </div>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {theme.events.map((evt) => (
                  ...
                  >
                    <span
                      className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase border mt-0.5 ${tierColors[evt.sourceTier]?.bg} ${tierColors[evt.sourceTier]?.text}`}
                    >
                      {evt.sourceTier}
                    </span>
                    <div className="flex-1 min-w-0">
