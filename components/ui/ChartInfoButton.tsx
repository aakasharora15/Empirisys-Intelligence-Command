          ...
          <div className="absolute right-0 top-10 w-72 bg-card/70 backdrop-blur-xl border border-card-border/50 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.12)] p-5 z-50 animate-in fade-in zoom-in-95 duration-200">
            ...
            {metrics && metrics.length > 0 && (
              <div className="space-y-2 border-t border-card-border pt-3">
                <span className="text-micro font-bold text-text-secondary uppercase tracking-wider block">How to read this data</span>
                {metrics.map((m, i) => (
                  <div key={i} className="bg-transparent rounded-lg p-2.5 border border-card-border/60">
                    <span className="text-micro font-bold text-text-primary block mb-0.5">{m.label}</span>
                    <span className="text-micro text-text-secondary leading-tight block">{m.desc}</span>
                  </div>
                ))}
              </div>
            )}
