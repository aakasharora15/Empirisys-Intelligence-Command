...
import { Competitor, DiscoveryLog, TriggerEvent } from '@/lib/db';
...
const COMP_ACTIVITY_FEED: Array<{
...
}> = [
  {
    ...
    detail:
      'LLM Categorization: Feature sprawl signal detected · Embedding Step: UI complexity index rising · Clustering: 3 overlapping module releases',
    time: '2 hours ago',
  },
  {
    id: 'cf2',
    ...
    detail:
      'Clustering: Zero predictive AI indicators in filing · Embedding distance from BOOST: 0.95 · Retrospective capture pattern reconfirmed',
    time: '5 hours ago',
  },
  {
    id: 'cf3',
    ...
    detail:
      'LLM Categorization: Substitute threat neutralised · Compliance gap: COMAH security audit failed · Re-engagement signal: High',
    time: '1 day ago',
  },
  {
    id: 'cf4',
    ...
    detail:
      'Cluster: Geographic expansion event · UK account resource dilution risk detected · Consulting gap widening: opportunity window open',
    time: '1 day ago',
  },
];

// ── Direct Rivals extra metrics ───────────────────────────────────────────────
interface RivalMetrics {
  sprawl: number;
  sprawlLabel: string;
  gap: number;
  gapLabel: string;
...
function getDirectRivalMetrics(name: string): RivalMetrics | null {
  ...
  if (n.includes('fennex') || n.includes('fenx'))
    return {
      sprawl: 72,
  ...
  if (n.includes('dnv') || n.includes('synergi'))
    return {
      sprawl: 55,
...
function clusterForCompetitor(comp: Competitor): CompCluster {
  ...
  if (n.includes('fennex') || n.includes('fenx') || n.includes('dnv') || n.includes('synergi'))
    return 'direct';
  return 'incumbent';
}
...
function getCompetitorFlags(name: string): CompetitorFlag[] {
  ...
  if (n.includes('enablon'))
    return [
      { label: 'Retrospective Only', variant: 'blue' },
      { label: 'No Predictive AI', variant: 'blue' },
      { label: 'Poor Glassdoor Reviews', variant: 'amber' },
    ];
  if (n.includes('eobs')) return [{ label: 'Partner / Adjacent Route', variant: 'green' }];
  if (n.includes('copilot') || n.includes('internal') || n.includes('diy'))
    return [
...
function getThreatConfig(score: number) {
  ...
  return {
    ...
    badge: 'bg-card/50 text-text-secondary border-card-border',
...
function getActivityStyle(source: ActivitySourceType) {
  if (source === 'INTERNAL')
    return { badge: 'bg-[#EF4444]/12 text-[#EF4444] border-[#EF4444]/25', accent: '#EF4444' };
  if (source === 'FENNEX')
    return { badge: 'bg-amber-400/12 text-amber-400 border-amber-400/25', accent: '#FBBF24' };
  if (source === 'DNV')
    return { badge: 'bg-amber-400/12 text-amber-400 border-amber-400/25', accent: '#FBBF24' };
    ...
    <>
      ...
      <div className="w-full bg-background/95 backdrop-blur-md border-b border-card-border">
        <div className="flex items-stretch">
          <div className="flex-shrink-0 px-4 py-2 bg-[#EF4444]/6 border-r border-[#EF4444]/15 flex items-center gap-4">
            ...
            <span className="text-base font-black text-[#EF4444] uppercase tracking-widest whitespace-nowrap">
              Live Triggers
            </span>
          </div>
              ...
              {[...triggerEvents, ...triggerEvents].map((event, i) => (
                <div key={i} className="flex items-center gap-4.5 whitespace-nowrap">
                  ...
                  <span className="text-base text-text-secondary">
                    {event.source} · {event.published_at}
                  </span>
          ...
          <div className="w-full flex justify-end gap-6 mt-8">
            <div className="flex items-center gap-4 pb-1">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-card/40 border border-card-border">
                ...
                <span className="text-base font-bold text-text-secondary uppercase tracking-widest">
                  Agent Sync Active
                </span>
              </div>
              <button
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="text-base font-bold text-text-primary">
                  {discoveryLogs.length} new competitor{discoveryLogs.length > 1 ? 's' : ''}{' '}
                  detected in the trailing 7 days.
                </p>
                <Link
                  href="/competitors"
                  className="text-lg text-accent hover:text-accent/80 font-bold mt-0.5 inline-block"
                >
      ...
      <div className="w-full px-4 md:px-8 pb-12 max-w-[1800px] mx-auto">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4 md:gap-5"
        >
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <p className="text-xs font-bold text-text-secondary uppercase tracking-widest leading-tight">
                    {card.title}
                  </p>
                  <div
                    className={cn(
                      'p-2 rounded-xl shrink-0',
                      card.danger ? 'bg-[#EF4444]/10 text-[#EF4444]' : 'bg-accent/10 text-accent',
                    )}
                  >
                ...
                <div>
                  <p className="text-5xl font-black text-text-primary leading-none tracking-tight font-sans mb-2">
                    {card.val}
                  </p>
          ...
          <motion.div
            variants={itemVariants}
            className="col-span-1 md:col-span-2 xl:col-span-2 glass-card p-6 md:p-8 rounded-3xl border border-white/5 flex flex-col"
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                <div className="h-2 w-2 rounded-full bg-amber-400" />
                <h3 className="text-xs font-bold text-text-secondary/80 uppercase tracking-widest">
                  Direct Rivals
                </h3>
                <div className="ml-2">
                  <ChartInfoButton
                    title="Direct Rivals"
                    description="Identifies competitors actively contesting your target accounts, highlighting product sprawl and capability gaps."
                  />
                </div>
              </div>
              <span className="text-xs font-bold tracking-widest text-amber-400 bg-amber-400/10 border border-amber-400/20 px-2 py-1 rounded-md uppercase">
                {directRivals.length} tracked
              </span>
            </div>
            <div className="space-y-4 overflow-y-auto max-h-[320px] pr-2 custom-scrollbar">
              {directRivals.map((comp) => {
                ...
                return (
                  <div
                    key={comp.id}
                    className="bg-background/40 border border-white/5 rounded-2xl p-5 hover:bg-white/[0.02] transition-colors"
                  >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-full bg-accent/10 border border-white/10 flex items-center justify-center overflow-hidden shrink-0 relative">
                          <img
                            src={
                              comp.logoUrl ??
                              `https://www.google.com/s2/favicons?domain=${(comp.website || '').replace('https://', '').split('/')[0]}&sz=64`
                            }
                            alt={comp.name}
                    ...
                    <div className="h-1.5 w-full bg-background rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${comp.threat_score}%`, backgroundColor: threat.bar }}
                      />
                    </div>
                  </div>
                );
          ...
          <motion.div
            variants={itemVariants}
            className="col-span-1 md:col-span-2 xl:col-span-2 glass-card rounded-3xl border border-white/5 flex flex-col overflow-hidden max-h-[500px]"
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="ml-2">
                  <ChartInfoButton
                    ...
                    description="Real-time ingestion of competitor press releases, product updates, and client signals categorized by LLM."
                  />
                </div>
              </div>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                <div className="ml-2">
                  <ChartInfoButton
                    title="Incumbent Systems"
                    description="Legacy capture platforms vulnerable to disruption by predictive AI models. Highly likely to contain retrospective data only."
                  />
                </div>
              </div>
              <span className="text-xs font-bold tracking-widest text-blue-400 bg-blue-400/10 border border-blue-400/20 px-2 py-1 rounded-md uppercase">
                {incumbents.length} tracked
              </span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto max-h-[320px] pr-2 custom-scrollbar">
              {incumbents.map((comp) => {
                ...
                return (
                  <div
                    key={comp.id}
                    className="bg-background/40 border border-white/5 rounded-2xl p-4 hover:bg-white/[0.02] transition-colors flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-blue-400/10 border border-white/10 flex items-center justify-center overflow-hidden shrink-0 relative">
                        ...
                        />
                        <span className="absolute inset-0 flex items-center justify-center text-blue-400 text-xs font-black z-0">
                          {comp.name.slice(0, 2).toUpperCase()}
                        </span>
                      </div>
          ...
          <motion.div
            variants={itemVariants}
            className="col-span-1 md:col-span-2 xl:col-span-2 glass-card rounded-3xl border border-white/5 flex flex-col overflow-hidden max-h-[400px]"
          >
            <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div className="flex items-center gap-3">
                ...
                <div className="ml-2">
                  <ChartInfoButton
                    title="HSE Pain Points"
                    description="Industry-wide Health, Safety, and Environment events or regulatory shifts that create immediate demand for Empirisys."
                  />
                </div>
              </div>
              <span className="text-xs font-bold text-text-secondary">
                {triggerEvents.length} active
              </span>
            </div>
            <div className="divide-y divide-white/5 overflow-y-auto flex-1 custom-scrollbar">
              {triggerEvents.map((event) => (
                <div
                  key={event.id}
                  className="p-6 flex items-start gap-4 hover:bg-white/[0.02] transition-colors"
                >
                  <span
                    className={cn(
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="ml-2">
                  <ChartInfoButton
                    ...
                    description="DIY AI initiatives, internal toolings, or generic copilots that pose high security risks and lack auditability."
                  />
                </div>
              </div>
              <span className="text-xs font-bold tracking-widest text-[#EF4444] bg-[#EF4444]/10 border border-[#EF4444]/20 px-2 py-1 rounded-md uppercase">
                {substitutes.length} tracked
              </span>
            </div>
            <div className="space-y-4 overflow-y-auto max-h-[320px] pr-2 custom-scrollbar">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[#EF4444] mt-0.5 font-bold uppercase tracking-wider">
                            {threat.label} RISK
                          </p>
                        </div>
                      </div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="ml-2">
                <ChartInfoButton
                  ...
                  description="Instant tactical kill-sheets outlining specific weaknesses and counter-arguments against major competitors to help sales win deals."
                />
              </div>
            </div>
              ...
              {[
                {
                  comp: 'FENNEX',
                  counter:
                    'Emphasize our consulting advisory layer and complain about their UI complexity.',
                  model: 'Complex SaaS Modules',
                },
                {
                  comp: 'ENABLON',
                  counter: 'Highlight predictive AI. Enablon is a legacy system of record.',
                  model: 'Legacy Enterprise License',
                },
                {
                  comp: 'DIY AI / COPILOT',
                  counter:
                    'Attack on security and auditability. Copilot prompts are not repeatable.',
                  model: 'Hidden Compute/Risk Costs',
                },
              ].map((bc) => (
                <div
                  key={bc.comp}
                  className="bg-background/40 border border-white/5 rounded-2xl p-5"
                >
                  <h4 className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-2">
                    {bc.comp}
                  </h4>
                  <p className="text-sm text-accent leading-relaxed font-semibold italic">
                    {bc.counter}
                  </p>
                </div>
              ))}
